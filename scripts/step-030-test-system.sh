...
if [ -f "$SCRIPT_UTILS" ]; then
    ...
    echo -e "${YELLOW}1. Verify the RNN-T model is working:${NC}"
    echo "   ./scripts/step-035-verify-rnnt-model.sh"
    echo ""
    echo -e "${YELLOW}2. Fix AWS credentials for S3 access:${NC}"
    echo "   ./scripts/step-037-fix-aws-credentials.sh"
    echo ""
    echo -e "${YELLOW}3. Test S3 transcription:${NC}"
